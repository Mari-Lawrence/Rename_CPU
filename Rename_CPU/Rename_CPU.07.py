...
# 配置日志记录
logging.basicConfig(filename="cpu_name_editor.log", level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
...
LANGUAGES = {
    "zh": {
        ...
        "invalid_input": "无效输入，请输入有效的 CPU 名称（仅限字母、数字、空格）。",
    ...
    "en": {
        ...
        "invalid_input": "Invalid input, please enter a valid CPU name (letters, numbers, spaces only).",
...
def modify_cpu_name(new_name):
    ...
    try:
        ...
        messagebox.showinfo(LANGUAGES[current_lang]["success"], LANGUAGES[current_lang]["restart_hint"])
    except Exception as e:
